...
Creature::Creature(World* world, const char* name, const char* desc, const char* unArmedWeapon, Room* initialRoom, int maxHealth, int startingHealth, int baseAttack_m, int baseAttack_M, int baseDefense_m, int baseDefense_M, int critChances, int missChances, bool canDie) :Entity(world, name, desc, (Entity*)initialRoom) {
	this->name = name;
	description = desc;
...
void Creature::Consume(const vector<string>args) {
...
void Creature::TakeAction() {
	...
	if (combat_target != nullptr && !IsStunned()) {
		ChaseAttacker();
		Println(name + " roars and attacks " + combat_target->name);
...
void Creature::Drop(const vector<string>args) {
	for (vector<string>::const_iterator it = args.begin(); it != args.cend(); ++it) {
		string entityName = *it;

		Entity* eToDrop = GetChildNamed(entityName.c_str());
		if (eToDrop != nullptr) {
